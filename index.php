...
function status($Red, $apikey)
 
	{
 
$fg = file_get_contents("https://amfcode.my.id/hopper?license_key=$apikey");
 
$ex = json_decode($fg, true);
 
$stat = $ex['result']['message'];
 
if($stat=="Apikey Valid"){
 
	} else if($stat=="Apikey Not Valid/Expired"){
 
		echo "$Red Apikey Anda Tidak Terdaftar,Silahkan Kontak Admin! WhatsApp 6285157792619\n\n"; die;
 ...
 
{
    ...
    $url = "https://amfcode.my.id/hopper?license_key=$apikey";
...
echo "$Green
	   ...
	   (___/(__)  (_____) (__) (____)(__)   (__) \n"."$White               Samsung Galaxy Free 3 Month offer\n\n\n";
        ...
        $resconfig = json_decode($getfile, true);
 
$subemail = $resconfig['email'];	
 
$apikey = $resconfig['apikey'];
 
$detail_info = getInfo($apikey);
 
$ressinfo = json_decode($detail_info, true);
 
$nama = $ressinfo['result']['nama'];
 
$expired = $ressinfo['result']['expired'];
 
$expired2 = date('d F Y',strtotime($expired));
...
echo "\n$Yellow Hallo : $White $nama";
 
echo "\n$Yellow Apikey Anda : $White $apikey\n$Yellow Expired : $White $expired2\n\n";
...
$email = randUser().$subemail;
 
$subpass = randUser()."@amfcode.net";
 
$register = signup($email,$subpass);
